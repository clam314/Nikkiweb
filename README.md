...
Nikke-db 的角色索引主要是英文名和资源 ID，没有完整中文字段。页面里的中文显示名来自预生成的 `data/name-locales.js`：用 GameKee 角色图鉴的中文名和 Live2D 资源 ID 建立 `c... -> 中文名` 映射，再用 DotGG/Nikke.gg 的角色索引补英文别名，找不到时才回退到 Nikke-db 的英文名。Fandom 和萌娘百科的 API/raw 读取在当前环境里不稳定，没有作为静态页面的运行时依赖。
